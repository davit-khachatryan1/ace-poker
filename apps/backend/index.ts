...
async function sendTableState(table: Table) {
  const playerCards = table.holeCards.map(cards => cards?.join('')).join('|')
  const playerBalances = Array(table.capacity).fill(1000).join('|')
  const state = `:${playerCards}:${playerBalances}`

  try {
    const response = await axios.post('https://rlbot-svc.prod.kashxa-infra.com/act', 
      new URLSearchParams({
        format: '3blinds-ante',
        state: state
      }), 
    ...
    console.log(`Sent table state for table ${table.id}:`, response.data)
  } catch (error) {
    console.error(`Error sending table state for table ${table.id}:`, error)
  }
}
...
server.post<{ Body: Table }>('/api/sendTableState', async (request, reply) => {
  const table = request.body
  await sendTableState(table)
  return { message: 'State sent successfully' }
})
