          ...
          >
            <template v-if="index === 1">
              <div class="absolute top-0 right-0 z-10 leading-[0]">
                <button class="bg-blue-500 text-white p-1 text-xs rounded-tr-lg rounded-bl-lg" @click="askAI">
                  Ask AI
                </button>
              </div>
            </template>
...
import { getTable, sendTableState } from '../api'
...
const askAI = async () => {
  ...
  try {
    const response = await sendTableState(table.value)
    alert(`Suggested strategy: ${response.message}`)
  } catch (error) {
